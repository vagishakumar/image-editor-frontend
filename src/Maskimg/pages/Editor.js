...
console.log(uploadImgUrl)
  ...
  const extractImage = () => {
    ...
    maskCanvas.toBlob((blob) => {
      ...
      uploadMaskImg(file).then(() => setIsExtracting(false),setIsLoading(false));;
    }, "image/png");
  };
  ...
  return (
    ...
    <div className="editor-container">
      <div className="editor-header">
        <div className="editor-text">AI Image Editor {isLoading&&Spinnerarea()} </div>
      </div>
          ...
          <ul className="menu">
            {menuItems.map((item) => (
              <li key={item.name} className={activeItem === item.name ? "active" : ""} onClick={() => setActiveItem(item.name)}>
                <div className="menu-item">{item.icon}<span>{item.name}</span></div>
              </li>
            ))}
          </ul>
        </div>
        <div className="box">
          ...
          <>
            <div className="button-group">
              <input className="prompt" ref={textRef} type="text" placeholder="Enter prompt..." onChange={handleChange}/>
              <Buttonvalue text="Generate" onClick={generateImage} disabled={!inputValue}
              />
            </div>
          </>
          )}
          {activeItem === "removebg" && 
          <div className="button-group">
              <Buttonvalue text="Remove Background"  onClick={removebg} disabled={!isUploading} />
          </div>}
          {activeItem === "modify" && (
            <>
          <div className="button-group">
            <input className="prompt" ref={textRef} type="text" placeholder="Enter prompt..."onChange={handleChange} />
              <Buttonvalue text="Extract Shape" 
               onClick={()=>{ setIsExtracting(true);extractImage();}}
               disabled={!isUploading}/>
              <Buttonvalue text="Modify"  onClick={modifyImage}
                  disabled={isExtracting}
              />
              <Buttonvalue text="Clear All" onClick={clearCanvas} />
              <Buttonvalue text={isErasing ? "Disable Eraser" : "EnaEraser"}   onClick={toggleEraser} />
          </div>
            </>
          )}
          {activeItem === "eraseobj" && (
          ...
          <div className="button-group">
              <Buttonvalue text="Extract Shape" 
              onClick={()=>{ extractImage();}} disabled={!isUploading}  />
              <Buttonvalue text="Erase Selected Image"
              onClick={eraseObj} disabled={isExtracting}/>
              <Buttonvalue text="Clear All" onClick={clearCanvas} />
                <Buttonvalue text={isErasing ? "Disable Eraser" : "Enable Eraser"}onClick={toggleEraser} />
          </div>
            </>
          )}
        </div>
